...
export namespace function_traits{
    ...
    // 1. Конечная цель (By Value): сигнатура R(Arg), где Arg и R — Compute::VectorValue
    ...
    // 2. Конечная цель (By Const Reference): поддержка (const Compute::VectorValue<>&)
    template<typename S, size_t In, size_t Out>
    ...
    concept ArrayOpIn = requires { typename Analyzer<Func>::Scalar; } && std::same_as<T, typename Analyzer<Func>::Scalar> && Analyzer<Func>::IN == IN_DIM;
    template<typename Func, typename T, size_t IN_DIM, size_t OUT_DIM>
    concept ArrayOpInOut = ArrayOpIn<Func, T, IN_DIM> && Analyzer<Func>::OUT == OUT_DIM;
}